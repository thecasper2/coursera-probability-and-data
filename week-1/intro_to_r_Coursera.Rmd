...
ggplot(arbuthnot, aes(x=year, y=boys/total)) +
  geom_line() +
  geom_point() +
  labs(x="year", y="Proportion of boys (%)")
```
...
names(present)
print(paste0("There are ", length(names(present))," variables"))
```
...
years <- range(present$year)
print(paste0("The data ranges from year ", years[1], " to year ", years[2]))
```
...
```{r most-total-births}
head(present[order(-present$total),], 1)$year
```
